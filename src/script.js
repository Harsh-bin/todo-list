      ...
      const ProgressBar = ({ percentage, displayPercentage }) => {
        const p = Math.max(0, Math.min(100, percentage));
        return (
          <div className="w-full">
            <div className="w-full bg-slate-200 dark:bg-white/10 rounded-full h-2 my-1 overflow-hidden">
                <div
            ...
            {displayPercentage && (
                <div className="text-center text-[0.6rem] text-slate-800 dark:text-slate-200 font-mono mt-1">
                    {Math.round(p)}%
                </div>
            )}
          </div>
      ...
      const LOCAL_STORAGE_KEYS = [
          'day_tasks', 'week_tasks', 'month_tasks', 'notes', 'target_dates', 'stars', 'streak', 'theme', 'is_compact'
      ];
      ...
      const SettingsDropdown = ({ onToggleTheme, onTriggerImport, onExport, onReset, theme, onToggleCompact, isCompact }) => {
                    ...
                    <div className="absolute right-0 mt-2 w-56 origin-top-right bg-white dark:bg-black rounded-md shadow-lg ring-1 ring-black dark:ring-slate-800 ring-opacity-5 focus:outline-none z-10">
                        <div className="py-1">
                            <button onClick={() => { onToggleTheme(); setIsOpen(false); }} className={menuItemClass}>
                             ...
                             <button onClick={() => { onToggleCompact(); setIsOpen(false); }} className={menuItemClass}>
                                <CompressIcon className="w-5 h-5"/>
                                Switch to {isCompact ? 'Expanded' : 'Compact'} View
                            </button>
    ...
    const TaskWidget = ({ period, tasks, onAreaClick, onCompleteSubTask, onRemoveTask, onRemoveSubTask }) => {
      ...
      const CalendarWidget = ({ currentDate, onHeaderClick }) => {
          ...
          return (
              <Widget className="w-full mx-auto">
      ...
      const NotesWidget = ({ notes, onAddNote, onEditNote, onDeleteNote }) => {
        ...
        return (
          <Widget className="flex-1 flex flex-col">
              ...
              <div className="space-y-2 flex-grow h-0 overflow-y-auto pr-2">
                      ...
                      <div key={note.id} className="group flex items-start justify-between gap-2 text-sm p-2 rounded hover:bg-slate-200/60 dark:hover:bg-white/10 transition-colors">
                        <div className="flex items-start gap-2 flex-grow overflow-hidden">
                            ...
                            <div className="flex-grow overflow-hidden">
                                {note.title && <p className="font-bold text-slate-800 dark:text-slate-200 truncate">{note.title}</p>}
                                {note.type === 'link' ? 
                                    ...
                                    <span className="text-slate-700 dark:text-slate-300 whitespace-pre-wrap break-words line-clamp-3">
                                        {note.content}
                                    </span>
                                }
                            </div>
        ...
        <Widget className="flex-shrink-0">
            <div className="flex items-center justify-center gap-x-3 text-md font-mono h-full">
                <span className="flex items-center gap-2 text-slate-800 dark:text-slate-200">
                  Streak <span className="text-amber-500 dark:text-yellow-400 font-bold">🔥 {streak.current}</span>
                </span>
    ...
    const CountdownItemWidget = ({ target, onDeleteTarget }) => {
        ...
        if (end > start) {
            ...
            percentageRemaining = (remainingDuration / totalDuration) * 100;
        } else if (now >= end) {
        ...
        return (
            <Widget className="relative p-2 flex-grow min-w-[180px] group">
                <div className="flex justify-between text-xs font-mono text-slate-800 dark:text-slate-300 mb-1">
                    ...
                    <span className="text-right">{target.daysLeft}d left</span>
                </div>
                <ProgressBar percentage={percentageRemaining} displayPercentage={true} />
                <button 
                    onClick={(e) => { e.stopPropagation(); onDeleteTarget(target.id); }} 
                    title="Delete Countdown" 
                    className="absolute top-1 right-1 p-1 rounded-full text-red-600 dark:text-red-500 hover:bg-red-500/10 opacity-0 group-hover:opacity-100 transition-opacity"
                >
                    <TrashIcon className="w-4 h-4"/>
                </button>
            </Widget>
    ...
    const CompactHeader = ({ currentDate, targets, onOpenNotes, onCalendarClick }) => {
        ...
        return (
            <Widget className="w-full">
                <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm">
                    <button onClick={onCalendarClick} className="font-bold text-slate-800 dark:text-slate-200 hover:underline">{formatCompactDate(currentDate)}</button>
                    <span className="text-slate-400 dark:text-slate-600">|</span>
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
                        {targets.map(t => (
                            <button onClick={onCalendarClick} key={t.id} className="underline decoration-slate-400 decoration-dotted">
                                {t.label} {t.daysLeft}d left
                            </button>
                        ))}
                         {targets.length === 0 && <button onClick={onCalendarClick} className="text-slate-500 dark:text-slate-400">Add Countdown</button>}
                    </div>
                    <div className="flex-grow"></div>
                    <button onClick={onOpenNotes} className="flex items-center gap-2 font-semibold text-slate-600 dark:text-slate-300 hover:text-black dark:hover:text-white">
      ...
      function App() {
        ...
        const [isCompact, setIsCompact] = useLocalStorage('is_compact', false);
        ...
        const handleResetsAndStreak = useCallback(() => {
          ...
          setStreak(newStreak);
        }, [dayTasks, weekTasks, monthTasks, starsData, streak, setDayTasks, setWeekTasks, setMonthTasks, setStarsData, setStreak, currentDate]);
        useEffect(() => {
        ...
        const addOrUpdateNote = (type, data, id = null) => {
          ...
          const title = typeof data === 'string' ? '' : data.title;
          ...
          if (id) {
            setNotes(prev => prev.map(n => n.id === id ? {...n, title: title.trim(), content: content.trim()} : n));
          } else {
            const newNote = { id: crypto.randomUUID(), type, title: title.trim(), content: content.trim() };
        ...
        return (
          <div className="min-h-screen p-2 sm:p-4 lg:p-6 flex flex-col space-y-4 max-w-7xl mx-auto">
            ...
            {isCompact ? (
                <CompactHeader
                    ...
                    onOpenNotes={() => openModal('view-notes')}
                    onCalendarClick={() => openModal('manage-targets')}
                />
            ) : (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    <div className="lg:col-span-1 space-y-4">
                        ...
                        <div className="flex flex-wrap gap-2">
                            ...
                            <Widget 
                                onClick={() => openModal('manage-targets')} 
                                className={`cursor-pointer flex flex-col items-center justify-center ${activeTargets.length > 0 ? 'w-24 h-24 p-0 flex-shrink-0' : 'flex-grow min-h-[64px]'}`}
                            >
                                <PlusIcon className={`mb-0 ${activeTargets.length > 0 ? 'w-10 h-10' : 'w-8 h-8'}`}/>
                                {activeTargets.length === 0 && <span className="text-slate-500 dark:text-slate-400">Add a countdown</span>}
                            </Widget>
                        </div>
                    </div>
        ...
        return (
            <>
                <div className="flex justify-between items-center p-4 border-b border-slate-300 dark:border-slate-800 flex-shrink-0">
                    ...
                    <div className="flex items-center gap-2">
                        <button onClick={() => { onSave({ title, content }, note?.id); }} className="flex items-center gap-2 p-2 rounded-lg text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors" title="Save Note">
                            <SaveIcon/>
                            <span>Save</span>
                        </button>
                ...
                <div className="flex flex-col h-full">
                    <input 
                        ...
                        onChange={e => setTitle(e.target.value)}
                        placeholder="Title"
                        className="w-full bg-transparent p-4 text-xl font-bold focus:outline-none placeholder-slate-500 flex-shrink-0"
                    ...
                    <textarea 
                        ...
                        className="w-full h-full bg-transparent p-4 pt-0 text-base resize-none focus:outline-none placeholder-slate-500"
                        autoFocus={!!note?.title}
                    />
                </div>
            </>
      ...
      const NotesModalContent = ({ notes, onAddNote, onEditNote, onDeleteNote, onClose }) => {
        const [isAddOpen, setIsAddOpen] = useState(false);
        const addRef = useRef(null);
        useEffect(() => {
            const handleClickOutside = (event) => {
        ...
        return (
            <>
                <div className="flex justify-between items-center p-4 border-b border-slate-300 dark:border-slate-800 flex-shrink-0">
                    <h2 className="text-xl text-slate-900 dark:text-slate-100 font-bold flex items-center gap-2"><NoteIcon />Notes</h2>
                    <div className="flex items-center gap-2">
                         <div className="relative" ref={addRef}>
                            <button onClick={() => setIsAddOpen(o => !o)} className="p-2 rounded-full text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-200 dark:hover:bg-white/10 transition-colors">
                                <PlusIcon className="w-6 h-6"/>
                            </button>
                            {isAddOpen && (
                                <div className="absolute right-0 mt-2 w-48 origin-top-right bg-white dark:bg-black rounded-md shadow-lg ring-1 ring-black dark:ring-slate-800 ring-opacity-5 focus:outline-none z-10">
                ...
                <div className="p-4 space-y-2 flex-grow overflow-y-auto">
                    {notes.length > 0 ? notes.map(note => (
                        <div key={note.id} className="group flex items-start justify-between gap-2 text-sm p-2 rounded hover:bg-slate-200/60 dark:hover:bg-white/10 transition-colors">
                            <div className="flex items-start gap-2 flex-grow overflow-hidden">
                                {note.type === 'link' ? <LinkIcon className="w-4 h-4 mt-1 text-cyan-500 dark:text-cyan-400 flex-shrink-0" /> : <TextIcon className="w-4 h-4 mt-1 text-slate-500 dark:text-slate-400 flex-shrink-0" />}
                                <div className="flex-grow overflow-hidden">
                                    {note.title && <p className="font-bold text-slate-800 dark:text-slate-200 truncate">{note.title}</p>}
                                    {note.type === 'link' ? <a href={note.content} target="_blank" rel="noopener noreferrer" className="text-cyan-600 dark:text-cyan-400 hover:underline break-all">{note.content}</a> : 
                                    <span className="text-slate-700 dark:text-slate-300 whitespace-pre-wrap break-words line-clamp-3">
                                        {note.content}
                                    </span>}
                                </div>
      ...
      const RenderModals = (props) => {
          const { modal, closeModal, modalData, targets, notes, onAddTask, onAddOrUpdateNote, onDeleteNote, onAddTarget, onDeleteTarget, openModal } = props;
          ...
          if (modal === 'add-text-note' || modal === 'edit-text-note') {
              return <Modal isOpen={true} onClose={closeModal} size="fullscreen">
                <NotepadModalContent 
                    note={modalData?.note} 
                    onSave={(data, id) => onAddOrUpdateNote('text', data, id)}
          ...
          if (modal === 'view-notes') {
            return <Modal isOpen={true} onClose={closeModal} size="fullscreen">
                <NotesModalContent
                    notes={notes}
                    onAddNote={(type) => { closeModal(); setTimeout(() => openModal(type === 'text' ? 'add-text-note' : 'add-link-note'), 50); }}
                    onEditNote={(note) => { closeModal(); setTimeout(() => openModal(note.type === 'text' ? 'edit-text-note' : 'edit-link-note', { note }), 50); }}
